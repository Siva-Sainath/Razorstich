...
def recommend(tick: int, contacts_used: int, method: str, hours_since_failure: float):
    """Policy recommendation — real Dueling DDQN when weights are available."""
    try:
        from policy_bridge import recommend_live

        return recommend_live(tick, contacts_used, method, hours_since_failure)
    except Exception:
        pass

    return _recommend_mock(tick, contacts_used, method, hours_since_failure)


def _recommend_mock(tick: int, contacts_used: int, method: str, hours_since_failure: float):
